...
const dbAdministration = require('./routes/db-administration');
const appMainView = require('./routes/news');
...
const router = function (config) {
  ...
  appRouter.use('/jquery', express.static( __dirname + '/../node_modules/jquery'));
  appRouter.use('/bootstrap', express.static( __dirname + '/../node_modules/bootstrap'));
  ...
  appRouter.param('news', function (req, res, next, id) {
  ...
  const appConfiguredRoutes = appMainView(config);
  appRouter.get('/', mongoMiddleware, appConfiguredRoutes.index);


  const dbConfiguredRoutes = dbAdministration(config);

  appRouter.get('/db', mongoMiddleware, dbConfiguredRoutes.index);
  appRouter.post('/db', mongoMiddleware, dbConfiguredRoutes.addDatabase);
  appRouter.delete('/:database', mongoMiddleware, dbConfiguredRoutes.deleteDatabase);
  appRouter.get('/db/:database', mongoMiddleware, dbConfiguredRoutes.viewDatabase);

  appRouter.post('/checkValid', mongoMiddleware, dbConfiguredRoutes.checkValid);

  // Collection level routes
  appRouter.get('/db/:database/compact/:collection', mongoMiddleware, dbConfiguredRoutes.compactCollection);
  appRouter.get('/db/:database/expArr/:collection', mongoMiddleware, dbConfiguredRoutes.exportColArray);
  appRouter.get('/db/:database/expCsv/:collection', mongoMiddleware, dbConfiguredRoutes.exportCsv);
  appRouter.get('/db/:database/reIndex/:collection', mongoMiddleware, dbConfiguredRoutes.reIndex);
  appRouter.post('/db/:database/addIndex/:collection', mongoMiddleware, dbConfiguredRoutes.addIndex);
  appRouter.get('/db/:database/export/:collection', mongoMiddleware, dbConfiguredRoutes.exportCollection);
  appRouter.get('/db/:database/dropIndex/:collection', mongoMiddleware, dbConfiguredRoutes.dropIndex);
  appRouter.get('/db/:database/updateCollections', mongoMiddleware, dbConfiguredRoutes.updateCollections);

  // GridFS
  appRouter.post('/db/:database/gridFS', mongoMiddleware, dbConfiguredRoutes.addBucket);
  appRouter.delete('/db/:database/gridFS/:bucket', mongoMiddleware, dbConfiguredRoutes.deleteBucket);

  appRouter.get('/db/:database/gridFS/:bucket', mongoMiddleware, dbConfiguredRoutes.viewBucket);
  appRouter.post('/db/:database/gridFS/:bucket', mongoMiddleware, dbConfiguredRoutes.addFile);
  appRouter.get('/db/:database/gridFS/:bucket/:file', mongoMiddleware, dbConfiguredRoutes.getFile);
  appRouter.delete('/db/:database/gridFS/:bucket/:file', mongoMiddleware, dbConfiguredRoutes.deleteFile);

  appRouter.get('/db/:database/:collection', mongoMiddleware, dbConfiguredRoutes.viewCollection);
  appRouter.put('/db/:database/:collection', mongoMiddleware, dbConfiguredRoutes.renameCollection);
  appRouter.delete('/db/:database/:collection', mongoMiddleware, dbConfiguredRoutes.deleteCollection);
  appRouter.post('/db/:database', mongoMiddleware, dbConfiguredRoutes.addCollection);

  // Document routes
  appRouter.post('/db/:database/:collection', mongoMiddleware, dbConfiguredRoutes.addDocument);
  appRouter.get('/db/:database/:collection/:document', mongoMiddleware, dbConfiguredRoutes.viewDocument);
  appRouter.put('/db/:database/:collection/:document', mongoMiddleware, dbConfiguredRoutes.updateDocument);
  appRouter.delete('/db/:database/:collection/:document', mongoMiddleware, dbConfiguredRoutes.deleteDocument);

  // Property routes
  appRouter.get('/db/:database/:collection/:document/:prop', mongoMiddleware, dbConfiguredRoutes.getProperty);
